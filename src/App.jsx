        ...
        <div onClick={() => setImagenOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, color: "#e2e8f0", letterSpacing: "-0.02em" }}>✦ Recreate with Imagen</div>
                <div style={{ fontSize: 11, color: "#4a5568", marginTop: 2 }}>
                  {imagenStep === 1 ? "Step 1 — Choose a product image" :
                   imagenStep === 2 ? "Step 2 — Select an image" :
                   imagenStep === 3 ? "Step 3 — Style your scene" :
                      ...
                      <input type="file" accept="image/jpeg,image/png,image/webp" style={{ display: "none" }}
                        ...
                        onChange={e => {
                          ...
                          reader.onload = ev => {
                            const dataUrl = ev.target.result;
                            const base64 = dataUrl.split(',')[1];
                            setImagenSelected({ src: dataUrl, base64, mimeType: file.type });
                            setImagenStep(3);
                          };
                          reader.readAsDataURL(file);
                        }}
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={async () => {
                          ...
                          setImagenParsing(true);
                          setImagenError('');
                          try {
                            const r = await fetch('/api/scrape', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ url: imagenUrlInput }),
                            });
                            const d = await r.json();
                            const imgs = (d.images || []).filter(src => src && src.startsWith('http')).slice(0, 12);
                            if (!imgs.length) setImagenError('No images found at this URL');
                            setImagenParsedImages(imgs);
                            if (imgs.length) setImagenStep(2);
                          } catch(e) {
                            setImagenError('Failed to parse URL');
                          }
                        ...
                        }}>{imagenParsing ? "Parsing..." : "Parse"}</button>
                      </div>
